...
Submit to `KSP-CKAN/NetKAN` (as of 2026-09-03 we are **not** indexed, so the first
submission is still ahead). Files live in that repo's `NetKAN/` directory and must be
named after the `identifier`, so ours is `NetKAN/BoundlessMissions.netkan`. There is no
CONTRIBUTING or PR template in the repo; the guide is the CKAN wiki's
[Adding a mod to the CKAN](https://github.com/KSP-CKAN/CKAN/wiki/Adding-a-mod-to-the-CKAN).
Note that repo requires contributions be made under **CC-0**, which applies to the metadata
file, not to this mod.

**Submit a metanetkan, not the netkan itself.** There are two ways to do this and they
differ in who owns the metadata afterwards:

- Submitting the full file means every later metadata change (homepage, tags,
  dependencies, a new `resources` entry) is another PR against their repo, reviewed and
  merged on their schedule. Only *releases* are automatic.
- Submitting a **metanetkan** stub means the file in their repo just points at ours, and
  `BoundlessMissions.netkan` **in this repo becomes the live metadata**. Change it here,
  push, and the indexer picks it up. One PR, ever.

The spec supports this explicitly: "*`.netkan` files which contain such a reference are
known as recursive netkans or metanetkans. They are primarily used so that mod authors can
provide authoritative metadata.*" The whole submission is then:
...
Four things about that URL and stub.

**Point at the raw GitHub URL, not at `boundlessmissions.com`.** The file already lives at
this repo's root, so nothing new is created and there is no second copy to drift. More
importantly it avoids the trap `GeneKerman.version` fell into: that file is generated by
`build.sh` but only goes live on a *site* deploy, which is exactly why it 404'd while
v1.0.0 was already published. A dead metanetkan URL is worse than a dead AVC URL, because
the indexer cannot read anything at all.

**Push before you submit.** The bot reads whatever that URL serves right now, so an
uncommitted local edit is invisible to it. Check with
`curl -s <the raw URL> | python3 -m json.tool` and confirm it is the metadata you mean.

**Keep the stub minimal.** Fields set in the stub *override* the target, so anything left
in it becomes a second source of truth. A metanetkan also may not point at another
metanetkan. `resources.metanetkan` is auto-filled.

**The trade-off is real.** Nothing reviews our metadata again, so a mistake propagates to
every CKAN user on the next index with no second pair of eyes. That is an acceptable trade
while the file is version-controlled and small, but it is a trade.

### The review checklist

The wiki page above carries the checklist a reviewer actually works through, and it is not
identical to the Spec. Ours against it:

| Item | State |
|------|-------|
| License should match the ZIP or forum thread | GPL-3.0, and `LICENSE` ships in the zip |
| **Homepage should point to the forum thread** | **Open.** We point at `boundlessmissions.com` |
| `$vref` set if the ZIP has a `.version` | `#/ckan/ksp-avc`, and it does |
| Tags | Set, all from the suggested vocabulary |
| Dependencies / conflicts | ModuleManager required; ClickThroughBlocker and JanitorsCloset recommended |

The homepage item is convention rather than spec (the Spec says only "the preferred landing
page", and mods like RealFuels carry no homepage at all), but it is what a reviewer reads,
so expect it raised. The forum thread is step 4 of the release order above and therefore
exists by the time this PR is opened. Options: point `homepage` at the thread and move the
site to `resources.manual`, which is defined as "the mod's manual" and fits the docs
section exactly; or argue for the site in the PR body with the thread in
`resources.discussions`. With a metanetkan in place either can be changed afterwards
without a PR, which is most of the reason to use one.
